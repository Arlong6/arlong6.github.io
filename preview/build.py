...
<head>
  ...
  <title>Tony — Apps (preview {V})</title>
...
def page(v, css, hero, rows):
    return HEAD.replace('{V}', v).replace('{CSS}', css + PV_CSS) + hero + '    <ul class="apps">\n' + ''.join(rows) + '    </ul>\n' + FOOT.replace('{V}', v)
...
'''
rows_a = [row(a, '' if a['id']=='ai-job-autofill' else f'<img class="shot{" wide" if a["kind"]=="wide" else ""}" src="shots/{a["id"]}.jpg" alt="{a["name"]} screenshot" loading="lazy">') for a in APPS]
open('preview/a.html','w').write(page('A', CSS_A, TITLE, rows_a))
...
open('preview/b.html','w').write(page('B', CSS_B, hero_b, rows_b))
